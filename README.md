# cmux-open-file

fish 上で `@` を押すとファイル fuzzy ピッカーが起動し、選んだファイルを cmux に拡張子別レイアウトで開く個人用 CLI。

## Requirements

- macOS
- [fish](https://fishshell.com/) 3.x
- [cmux](https://cmux.app/)
- Node.js 18+
- `nvim` が `PATH` にあること

## Install

```bash
npm install -g cmux-open-file
```

`~/.config/fish/conf.d/cmux-open-file.fish` が自動配置される。fish を再起動するか新しいタブを開けば有効化される。

## Usage

fish のコマンドラインが空のときに `@` を押すと、自動で `@ ` が挿入されて補完候補（カレントディレクトリ配下のファイル一覧、fuzzy ランク順）が即座に表示される。

| キー | 動作 |
|---|---|
| 文字入力 | インクリメンタル fuzzy 検索（fish ネイティブの補完絞り込み） |
| Tab | 候補表示・選択肢を順に切り替え |
| ↑ / ↓ | 候補移動 |
| Enter | 確定して開く |
| Esc / Ctrl+C | キャンセル |

確定すると fish が `@ <path>` を実行し、拡張子に応じて cmux 上に新ペイン (右側) が1つ作られ、その中に2つのサーフェスが並ぶ:

- `.md` → nvim サーフェス + markdown プレビューサーフェス
- `.html` → nvim サーフェス + ブラウザサーフェス
- その他 → nvim サーフェスのみ

検索対象から `node_modules/` 配下は除外される。隠しファイル (`.git/` 含む) は対象。

## Uninstall

```bash
npm uninstall -g cmux-open-file
rm ~/.config/fish/conf.d/cmux-open-file.fish
```

## License

MIT
